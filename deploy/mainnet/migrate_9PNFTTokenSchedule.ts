...
const { waitForTx, verifyContract, loadDB, saveDB, upgradeContract } = helpers;
...
async function deploy() {
    ...
    // 
    //add schedules
    let pNFTToken = (await hre.ethers.getContractAt('PNFTToken', deployData.pNFTToken.address)) as PNFTToken;
    let numSchedules = await pNFTToken.getVestingSchedulesCount();

    if (numSchedules.eq(0)) {
        ...
        await waitForTx(
            ...
            'pNFTToken.createVestingSchedule core'
        )
        ...
        await waitForTx(
            ...
            'pNFTToken.createVestingSchedule reward'
        )
    }
}
